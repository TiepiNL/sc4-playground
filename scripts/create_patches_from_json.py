...
def write_patch_file(filename: str, patch_instance_id: int, targets: list):
    ...
    Args:
        ...
        targets: List of target exemplars (dictionaries or legacy tuples)
    """
    with open(filename, 'wb') as f:
        ...
        for target in targets:
            if isinstance(target, dict):
                ...
                # Legacy tuple format (group_id, instance_id)
                group_id, instance_id = target
...
def generate_patched_lots_file(grouped_targets):
    """
    Generate a text file listing all ExemplarName values that will be patched.
    
    Args:
        grouped_targets: Dictionary of grouped lot targets by zone/wealth combination
    """
    patched_lots_file = os.path.join(OUTPUT_DIR, "patched_lots.txt")
    
    # Collect all unique exemplar names from all groups
    all_exemplar_names = set()
    
    for group_name, targets in grouped_targets.items():
        for target in targets:
            if 'exemplar_name' in target:
                all_exemplar_names.add(target['exemplar_name'])
    
    # Sort names alphabetically for consistent output
    sorted_names = sorted(all_exemplar_names)
    
    # Write to file
    try:
        with open(patched_lots_file, 'w', encoding='utf-8') as f:
            f.write(f"# SimCity 4 Patched Lots List\n")
            f.write(f"# Generated by sc4-playground patch creation system\n")
            f.write(f"# Total lots patched: {len(sorted_names)}\n")
            f.write(f"# These lots will be blocked from growing automatically\n")
            f.write(f"#\n")
            
            for name in sorted_names:
                f.write(f"{name}\n")
        
        print(f"Generated patched lots list: {patched_lots_file}")
        print(f"Listed {len(sorted_names)} unique lot names")
        
    except Exception as e:
        print(f"Warning: Could not generate patched lots file: {e}")
...
def main():
        ...
        for purpose in purpose_values:
            for wealth in wealth_values:
                ...
                try:
                    ...
                    # Add to the appropriate patch file group with exemplar name
                    grouped_targets[filename].append({
    ...
    # Generate patched lots text file
    generate_patched_lots_file(grouped_targets)
